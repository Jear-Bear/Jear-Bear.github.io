...
export const ENTITIES = {
  ...
  payments: {
    ...
    fields: {
      ...
      fees: money('Fees ($)'),
      net: money('Net ($)'),
      invoice_no: text('Invoice number', 30),
      notes: long('Notes', 1000),
    },
  },
  links: {
    table: 'links', label: 'Tracked link',
    fields: {
      slug: text('Short name', 40, { required: true, pattern: /^[a-z0-9](?:[a-z0-9-]{0,38}[a-z0-9])?$/ }),
      target: { type: 'url', label: 'Goes to', required: true },
